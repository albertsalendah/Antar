...
func (h *Handler) Register(c *gin.Context) {
	...
	_, err = h.db.Exec(context.Background(),
		`INSERT INTO rider_profiles (id, full_name, phone_number, email, created_at, updated_at)
		 VALUES ($1, $2, $3, $4, $5, $5) ON CONFLICT (id) DO NOTHING`,
		result.UserID, req.FullName, req.PhoneNumber, req.Email, time.Now(),
	)
	if err != nil {
		slog.Error("Failed to insert rider_profile", "user_id", result.UserID, "error", err)
...
func (h *Handler) Login(c *gin.Context) {
	...
	h.db.QueryRow(context.Background(),
		`SELECT full_name FROM rider_profiles WHERE id = $1`, authResp.User.ID,
	).Scan(&fullName)
	response.Success(c, gin.H{
		"access_token":  authResp.AccessToken,
		"refresh_token": authResp.RefreshToken,
		"rider_id":      authResp.User.ID,
		"full_name":     fullName,
	})
}
...
func (h *Handler) NearbyDrivers(c *gin.Context) {
	...
	if vehicleTypeID != nil {
		vehicleTypeClause = fmt.Sprintf("AND dv.vehicle_type_id = %d", *vehicleTypeID)
	}
	...
	query := fmt.Sprintf(`
		...
		LIMIT 50`, vehicleTypeClause)

	rows, err := h.db.Query(context.Background(), query, lng, lat, islandID, searchRadius)
	if err != nil {
...
func (h *Handler) GetActiveTrip(c *gin.Context) {
	...
	err := scanTrip(
		h.db.QueryRow(context.Background(),
			tripSelect+`
			 WHERE t.rider_id = $1
			   AND t.status IN ('requested','offered','agreed','in_progress')
